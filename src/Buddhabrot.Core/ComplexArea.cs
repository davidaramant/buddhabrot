...
public readonly record struct ComplexArea(Interval RealInterval, Interval ImagInterval)
{
	...
	public bool Contains(Complex number) =>
		RealInterval.Contains(number.Real) && ImagInterval.Contains(number.Imaginary);

	public bool OverlapsWith(ComplexArea otherArea) =>
		RealInterval.OverlapsWith(otherArea.RealInterval) && ImagInterval.OverlapsWith(otherArea.ImagInterval);
	...
	public ComplexArea Scale(double scale) => new(RealInterval.Scale(scale), ImagInterval.Scale(scale));
