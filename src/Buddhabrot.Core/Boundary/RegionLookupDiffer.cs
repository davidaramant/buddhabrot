...
{
	private static readonly IReadOnlyDictionary<(Type Left, Type Right), Type> TypeMappings = new Dictionary<
	...
	public static RegionLookup Diff(RegionLookup left, RegionLookup right)
	{
		...
		diffTree.Add(Diff(left.Nodes, left.Nodes.Last(), right.Nodes, right.Nodes.Last(), diffTree, cache));
	...
	)
	{
		...
		if (!left.IsLeaf && !right.IsLeaf)
		{
			var sw = Diff(
				leftTree,
				leftTree[left.GetChildIndex(Quadrant.SW)],
				rightTree,
				rightTree[right.GetChildIndex(Quadrant.SW)],
				diffTree,
				cache
			);
			...
			var se = Diff(
				leftTree,
				leftTree[left.GetChildIndex(Quadrant.SE)],
				rightTree,
				rightTree[right.GetChildIndex(Quadrant.SE)],
				diffTree,
				cache
			);
			...
			var nw = Diff(
				leftTree,
				leftTree[left.GetChildIndex(Quadrant.NW)],
				rightTree,
				rightTree[right.GetChildIndex(Quadrant.NW)],
				diffTree,
				cache
			);
			...
			var ne = Diff(
				leftTree,
				leftTree[left.GetChildIndex(Quadrant.NE)],
				rightTree,
				rightTree[right.GetChildIndex(Quadrant.NE)],
				diffTree,
				cache
			);
	...
	private static Node MakeQuad(List<Node> diffTree, Cache cache, Node sw, Node se, Node nw, Node ne)
	{
	...
	public static Type CondenseRegionType(Type sw, Type se, Type nw, Type ne)
	{
		uint groups = 0;
