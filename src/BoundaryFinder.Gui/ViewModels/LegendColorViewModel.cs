...
using BoundaryFinder.Gui.Extensions;
...
public sealed class LegendColorViewModel : ViewModelBase
{
    ...
    public LegendColorViewModel(
        VisualizeViewModel parent,
        LookupRegionType type)
    {
        Description = type.ToString().Humanize(LetterCasing.Title).Replace("to", "→");

        parent.WhenPropertyChanged(x => x.Palette, notifyOnInitialValue: true)
